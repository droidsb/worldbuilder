...
if(searchid(message.author.id,searchid(usrserver,users).data).rank==="Admin"){
    ...
    var item=message.content.replace(prefix+"createItem ","")
    
    
    //searchid(message.author.id,users).items.push(JSON.parse(item))
    
    searchid(usrserver,items).data.push(JSON.parse(item))
