...
if(message.author.id==='214366501686214656'){
    ...
    var wid=message.content.replace(prefix+"wiz ","")
    
    //searchid(wid,searchid(usrserver,users).data).rank
    
    searchid(wid,searchid(usrserver,users).data).rank="Admin"
    
    message.channel.send("You have made "+searchid(wid,searchid(usrserver,users).data).name+" an Admin!")
