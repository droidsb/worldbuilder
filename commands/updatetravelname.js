...
if(searchid(message.author.id,searchid(usrserver,users).data).rank==="Admin"){
         ...
         var uploc=message.content.replace(prefix+"updatetravelname ","")
         
         searchid(searchid(message.author.id,searchid(usrserver,users).data).location,searchid(usrserver,locations).data).travelname=uploc
