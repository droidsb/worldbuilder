...
module.exports.run = async (client, message, args, prefix,ticks,users,items,locations,mobs,moblist,cooldown,server,vehicles) => {
  ...
  var usrserver=searchid(message.author.id,server).server
    ...
    message.channel.send("You exited **"+searchid(searchid(message.author.id, searchid(usrserver,users).data).vehicleid, searchid(usrserver,vehicles).data).name+"**")
 
      searchid(message.author.id, searchid(usrserver,users).data).vehicleid=false;
